...
async function activateExtension(extension, options) {
  ...
  console.log(`[Extensions] Activating ${extension.id} from ${extension.webEntrypoint}...`);
  ...
  console.log(`[Extensions] ${extension.id} activated successfully.`);
...
export function getExtensionCompletions(languageId) {
  ...
  // Handle React-aware language matching
  const targetIds = [normalizedLanguageId];
  if (normalizedLanguageId === 'javascript' || normalizedLanguageId === 'typescript') {
    targetIds.push(`${normalizedLanguageId}react`);
  } else if (normalizedLanguageId.endsWith('react')) {
    targetIds.push(normalizedLanguageId.replace('react', ''));
  }

  const allItems = [];
  targetIds.forEach(id => {
    const bucket = runtimeState.completionItems.get(id);
    if (bucket?.size) {
      allItems.push(...[...bucket.values()].flat());
    }
  });
