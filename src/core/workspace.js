...
function toAbsoluteWorkspacePath(rootPath = '', relativePath = '') {
  const normalizedRoot = String(rootPath || '').replace(/\\/g, '/').replace(/\/+$/, '');
  const rawRelative = String(relativePath || '').replace(/\\/g, '/');
  
  // If the path is already an absolute path (Windows drive or Unix root), return it directly
  if (/^[a-zA-Z]:\//.test(rawRelative) || rawRelative.startsWith('/')) {
    return rawRelative;
  }
...
const workspace = {
  ...
  getStructureSnapshot() {
    ...
    function visit(node) {
      if (node.path !== 'root') {
          const isAbsolute = /^[a-zA-Z]:[\\/\\\\]/.test(node.path) || node.path.startsWith('/');
          if (!isAbsolute) {
            entries.push({
              path: node.path,
              type: node.type,
  ...
  mapDesktopTreeNode(entry, parentPath = 'root', rootSystemPath = '', isLoaded = false) {
    ...
    if (entry.type === 'folder') {
      // A native folder response contains its real subtree. Empty folders are
      // still considered loaded so clicking them toggles immediately instead
      // of getting stuck in a permanent "open/loading" state.
      ...
      return {
        ...
        isLoaded: isLoaded || Array.isArray(entry.children),
      };
    }
  ...
  async reloadTree() {
    ...
    for (const rootConfig of this.roots) {
      if (this.adapter === 'tauri' && rootConfig.systemPath) {
        // Read one coherent native subtree. Mixing a shallow root response with
        // later lazy responses was allowing child nodes to be rendered as root
        // siblings after a refresh.
        const children = await desktopReadTree(rootConfig.systemPath);
        const rootNode = {
    ...
    this.resetWorkspaceView({
      keepTabs: true,
      selectedPath: this.findNode(this.selectedNodePath) ? this.selectedNodePath : (this.tree.length ? 'root' : null),
  ...
  async loadDirectory(path, tree = this.tree, options = {}) {
    const { force = false } = options;
    const node = this.findNode(path, tree);
    if (!node || node.type !== 'folder' || (!force && node.isLoaded && Array.isArray(node.children))) {
      return;
    }
    ...
    try {
      if (this.adapter === 'tauri') {
        ...
        // A recursive native result keeps a folder's actual descendants
        // together instead of rebuilding it from a shallow directory read.
        const children = await desktopReadTree(nativePath);
  ...
  findNode(path, nodes = this.tree) {
    // Desktop roots retain their absolute native path, while workspace
    // commands use `root`. Resolve that alias only at the top level.
    if (path === 'root' && nodes === this.tree) {
  ...
  async revealNodeInExplorer(path) {
    if (this.adapter !== 'tauri') return;
    const node = this.findNode(path);
    let nativePath = node?.nativePath || (node ? toAbsoluteWorkspacePath(this.rootSystemPath, node.path) : null);
    if (!nativePath && (path === 'root' || !path)) {
      nativePath = this.rootSystemPath;
    }
  ...
  async toggleFolder(path) {
    ...
    // Never mutate the existing Set. Explorer rows are memoized and need a
    // new reference whenever a folder changes between collapsed and expanded.
    const nextExpandedPaths = new Set(this.expandedPaths);
    const isExpanded = nextExpandedPaths.has(path);
    this.expansionVersion += 1;
    if (isExpanded) {
      // Closing must always win, including for an empty folder.
      nextExpandedPaths.delete(path);
      this.expandedPaths = nextExpandedPaths;
      node.open = false;
    } else {
      // Tauri's workspace reader already provides the complete recursive
      // branch. Expansion must be an immediate UI state transition, not a
      // second filesystem read that can replace the branch while React draws.
      nextExpandedPaths.add(path);
      this.expandedPaths = nextExpandedPaths;
      node.open = true;
      publishToggleState();
      if (!node.isLoaded) {
        await this.loadDirectory(path, this.tree);
      }
    }
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('focus', () => {
    ...
    if (workspace.adapter === 'tauri' && workspace.roots.length > 0) {
      const now = Date.now();
      const lastExplorerInteractionAt = Number(window.__tilderExplorerInteractionAt || 0);
      if (now - lastExplorerInteractionAt < 1200 || now - lastFocusReloadAt < 1500 || focusReloadPromise) {
        return;
      }

      lastFocusReloadAt = now;
      const expandedBeforeFocus = new Set(workspace.expandedPaths || []);
      const expansionVersionBeforeFocus = workspace.expansionVersion;
      focusReloadPromise = workspace.reloadTree()
        .then(() => {
          // Do not put an old expansion snapshot back after the user has
          // clicked a folder while the filesystem refresh was in flight.
          if (workspace.expansionVersion !== expansionVersionBeforeFocus) {
            return;
          }
          workspace.expandedPaths = expandedBeforeFocus;
          const applyOpenState = (nodes = []) => {
            nodes.forEach((node) => {
              if (node?.type === 'folder') {
                node.open = node.path === 'root' || workspace.expandedPaths.has(node.path);
                applyOpenState(node.children || []);
              }
            });
          };
          applyOpenState(workspace.tree);
          if (workspace.tree) {
