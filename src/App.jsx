...
import './App.css';
import tilderIcon from '../Tilder_icon.png';
...
import TestRunner from './components/SideBar/Main Components/TestRunner/TestRunner.jsx';
...
import PromptDialog from './components/PromptDialog/PromptDialog.jsx';
...
import WorkspaceTrustModal from './components/WorkspaceTrustModal/WorkspaceTrustModal.jsx';
import AILensModal from './components/AILensModal/AILensModal.jsx';
...
import defaultSettings, { mergeWithDefaultSettings } from './components/Settings/defaultSettings.js';
...
import { fetchRunnerLanguages, formatLocalRunResult, formatRunResult, resolveRunnerLanguage, runCode, getRunCommandForTab, syncTerminalWorkspaceRoot } from './core/codeRunner.js';
import { getDetectedRuntimes } from './core/runtimeScanner.js';
...
const RECENT_FILES_STORAGE_KEY = 'tilderRecentFiles';
const RECENT_WORKSPACES_STORAGE_KEY = 'tilderRecentWorkspaces';

function readRecentEntries(storageKey) {
  try {
...
function App() {
  const [backpackSnippets, setBackpackSnippets] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('tilder-backpack-snippets') || '[]');
    } catch {
      return [];
    }
  });
  ...
  // Global UI Sound Hook for Tahoe Theme
  useEffect(() => {
    const handleGlobalClick = (e) => {
      if (document.body.dataset.theme !== 'tilder-tahoe') return;
      
      const target = e.target;
      
      // If it's a toggle switch
      if (target.matches('input[type="checkbox"], input[type="radio"], .tilderSettingsToggleSlider')) {
        soundManager.playClick();
        return;
      }
      
      // If it's a tab or sidebar item
      if (target.closest('.tab, .sidebarIconBar button, .tilderSettingsSidebarItem, .explorer-toolbar-btn')) {
        soundManager.playTick();
        return;
      }
      
      // If it's a button or general interactive element
      if (target.closest('button, .go-popup-result, .tilderSettingsCardTitleRow')) {
        soundManager.playClick();
        return;
      }
    };
  ...
  useEffect(() => {
    if (isDesktopRuntime()) {
      fetchLocalBackpack().then(data => {
        if (data && data.snippets && data.snippets.length > 0) {
          setBackpackSnippets(data.snippets);
          localStorage.setItem('tilder-backpack-snippets', JSON.stringify(data.snippets));
        }
      }).catch(err => console.warn('Failed to fetch local backpack:', err));
    }
  }, []);

  const handleBackpackSnippetsChange = useCallback((nextSnippets) => {
    setBackpackSnippets(nextSnippets);
    localStorage.setItem('tilder-backpack-snippets', JSON.stringify(nextSnippets));
    if (isDesktopRuntime()) {
      saveLocalBackpack(nextSnippets).catch(err => console.warn('Failed to save local backpack:', err));
    }
  }, []);

  const [settings, setSettings] = useState(() => {
    const saved = localStorage.getItem('editorSettings');
    return saved ? mergeWithDefaultSettings(JSON.parse(saved)) : mergeWithDefaultSettings(defaultSettings);
  });
  const [recentFiles, setRecentFiles] = useState(() => readRecentEntries(RECENT_FILES_STORAGE_KEY));
  const [recentWorkspaces, setRecentWorkspaces] = useState(() => readRecentEntries(RECENT_WORKSPACES_STORAGE_KEY));
  ...
  const [pendingTrustFolder, setPendingTrustFolder] = useState(null);
  ...
  const [createFileRequestNonce, setCreateFileRequestNonce] = useState(0);
  const [renameRequestNonce, setRenameRequestNonce] = useState(0);
  const [terminalOpen, setTerminalOpen] = useState(false);
  const [terminalHeight, setTerminalHeight] = useState(220);
  const [terminalMaximized, setTerminalMaximized] = useState(false);
  const [bottomPanelView, setBottomPanelView] = useState('terminal');
  ...
  const [aiLensTarget, setAiLensTarget] = useState(null);
  ...
  const [startupSplashPhase, setStartupSplashPhase] = useState('visible');
  ...
  const [detectedRuntimesState, setDetectedRuntimesState] = useState({});
  const [outputEntries, setOutputEntries] = useState([]);
  const [debugConsoleEntries, setDebugConsoleEntries] = useState([]);
  const [realDebugStack, setRealDebugStack] = useState([]);
  const [realDebugVariables, setRealDebugVariables] = useState([]);
  ...
  const escapeCurrentModeRef = useRef(() => {});
  ...
  const navigationHistoryRef = useRef({ current: null, back: [], forward: [], suppress: false });
  const zenRestoreRef = useRef(null);
  ...
  const lspBridgeCacheRef = useRef(new Map());
  const activeLspBridgeKeyRef = useRef('');
  ...
  useEffect(() => {
    window.tilderExecuteTerminalCommand = (cmd) => {
      if (terminalApiRef.current && terminalApiRef.current.executeCommand) {
        terminalApiRef.current.executeCommand(cmd);
        return true;
      }
      return false;
    };
    return () => {
      delete window.tilderExecuteTerminalCommand;
    };
  }, []);

  useEffect(() => {
    localStorage.setItem(RECENT_FILES_STORAGE_KEY, JSON.stringify(recentFiles));
    localStorage.setItem(RECENT_WORKSPACES_STORAGE_KEY, JSON.stringify(recentWorkspaces));
    if (isDesktopRuntime()) {
      desktopUpdateJumpList(recentFiles, recentWorkspaces);
    }
  ...
  useEffect(() => {
    getDetectedRuntimes().then((runtimes) => {
      setDetectedRuntimesState(runtimes || {});
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    }, 400); // Reduced from 1100ms for much faster startup
    const hideTimer = window.setTimeout(() => {
      setStartupSplashPhase('hidden');
    }, 700); // Reduced from 1550ms for much faster startup
  ...
  useEffect(() => {
    const activeTheme = settings?.theme?.active || 'tilder-formal';
    const aesthetics = settings?.aesthetics || {};
    applyTheme(activeTheme, aesthetics);
  }, [settings?.theme, settings?.aesthetics]);
  ...
  useEffect(() => {
    // Handle startup arguments passed from Tauri via Jump List
    if (typeof window !== 'undefined') {
      const startupWorkspace = window.__TILDER_STARTUP_WORKSPACE__;
      const startupFile = window.__TILDER_STARTUP_FILE__;

      if (startupWorkspace && isDesktopRuntime()) {
        workspace.roots = [{
          id: startupWorkspace,
          handle: { kind: 'desktop-root', name: startupWorkspace.split(/[\\/]/).pop() || startupWorkspace },
          systemPath: startupWorkspace,
          name: startupWorkspace.split(/[\\/]/).pop() || startupWorkspace
        }];
        workspace.resetWorkspaceView({ keepTabs: false, selectedPath: 'root' });
        workspace.reloadTree().then(() => refresh());
        window.__TILDER_STARTUP_WORKSPACE__ = undefined;
      }

      if (startupFile && isDesktopRuntime()) {
        // Find the node if it's within roots, otherwise just open it externally
        workspace.openFile({
          type: 'file',
          name: startupFile.split(/[\\/]/).pop() || startupFile,
          path: startupFile,
          nativePath: startupFile,
          isDraft: false,
        }).then(() => refresh());
        window.__TILDER_STARTUP_FILE__ = undefined;
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && isDesktopRuntime()) {
      try {
        ...
        const workspaces = JSON.parse(localStorage.getItem(wsKey) || '[]');
        desktopUpdateJumpList(files, workspaces);
      } catch (err) {
        console.error('Failed to initialize jump list on startup:', err);
      }
    }
  }, []);

  // Auto-check for updates on startup (silent, non-manual)
  useEffect(() => {
    if (typeof window !== 'undefined' && isDesktopRuntime()) {
      const timer = window.setTimeout(() => {
        handleCheckForUpdates(false);
      }, 5000); // 5s delay so it doesn't block startup
      return () => window.clearTimeout(timer);
    }
  }, []);

  // Intercept all external link clicks to open them in the external browser
  useEffect(() => {
    const handleGlobalClick = (e) => {
      ...
      if (anchor && anchor.href) {
        ...
        if (url.startsWith('http://') || url.startsWith('https://')) {
          ...
          }).catch(err => console.error('Failed to open link via API:', err));
        }
      }
    };
  ...
  useEffect(() => {
    const handleAutoSave = (mode) => {
      const active = workspace.getActiveTab();
      if (active && active.isDirty && active.handle && !active.isUntitled) {
        workspace.saveTab(active.id).then(() => refresh());
      }
    };

    const handleWindowBlur = () => {
      if (settings?.files?.autoSave === 'onWindowChange') {
        handleAutoSave();
      }
    };

    const handleFocusOut = (event) => {
      if (settings?.files?.autoSave === 'onFocusChange') {
        // Only save if focus moves OUTSIDE the main editor area
        const editorArea = document.querySelector('.maincodearea');
        if (editorArea && !editorArea.contains(event.relatedTarget)) {
          handleAutoSave();
        }
      }
    };
  ...
  useEffect(() => {
    const handleConsoleMsg = (msg) => {
      setDebugConsoleEntries((current) => [
        ...current,
        {
          id: `dbg-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
    ...
    const handlePaused = async (params) => {
      ...
      setRealDebugStack(debugApi.getCallStack());
      const vars = await debugApi.getScopeVariables().catch(() => []);
      setRealDebugVariables(vars);
    };
    ...
    const handleResumed = () => {
      ...
      setRealDebugStack([]);
      setRealDebugVariables([]);
    };

    debugApi.on('console.message', handleConsoleMsg);
    debugApi.on('Debugger.paused', handlePaused);
    debugApi.on('Debugger.resumed', handleResumed);

    return () => {
      debugApi.off('console.message', handleConsoleMsg);
      debugApi.off('Debugger.paused', handlePaused);
      debugApi.off('Debugger.resumed', handleResumed);
    };
  }, []);
  ...
  useEffect(() => {
    const onPaused = (params) => {
      ...
      const { lineNumber, scriptId } = topFrame.location;
      const displayLine = lineNumber + 1;
      
      setDebugSession((current) => ({
        ...current,
        status: 'paused',
        message: `Paused at ${topFrame.functionName || 'anonymous'}`,
      }));
      
      // Update editor visuals
      refreshDebugExecutionDecorations(getPreferredEditor(), getPreferredMonaco(), displayLine, []);
      
      // Proactively reveal the line in the editor
      // (This would require scriptId to path resolution which we'll add later)
    };
    ...
    const onResumed = () => {
      ...
      // Clear decorations
      refreshDebugExecutionDecorations(getPreferredEditor(), getPreferredMonaco(), null);
    };
    ...
    debugApi.on('Debugger.paused', onPaused);
    debugApi.on('Debugger.resumed', onResumed);
    debugApi.on('sessionClosed', onSessionClosed);

    return () => {
      debugApi.off('Debugger.paused', onPaused);
      debugApi.off('Debugger.resumed', onResumed);
      debugApi.off('sessionClosed', onSessionClosed);
    };
  }, []);
  ...
  useEffect(() => {
    if (!goOverlay.open || ['line', 'definition', 'reference'].includes(goOverlay.mode)) {
      return undefined;
    }
  ...
  useEffect(() => {
    ...
    fetchAuthSession()
      .then((session) => {
        ...
        if (sId) {
          localStorage.setItem('tilder_session_id', sId);
        }
  ...
  function openSearchPanel(request = null) {
    ...
    const nextRequest = request ? { ...request } : { mode: 'content', showReplace: false };
  ...
  async function copyTextToClipboard(text, successMessage = 'Copied to clipboard.') {
    if (!text) {
      pushNotification('Nothing to copy.', 'warning');
      return false;
    }
    ...
    try {
      ...
      pushNotification('Could not copy to clipboard.', 'warning');
      return false;
    }
  }

  function getSelectedEditorText(maxLength = 160) {
    ...
    return String(model.getValueInRange(selection) || '').trim().slice(0, maxLength);
  }

  function getExplorerTargetPath() {
    ...
    return focusedTab.path || focusedTab.id || null;
  }

  async function handleCopyExplorerPath({ relative = false } = {}) {
    const targetPath = getExplorerTargetPath();
    if (!targetPath) {
      pushNotification('Select a file or folder first.', 'warning');
      return false;
    }

    return copyTextToClipboard(
      workspace.getDisplayPath(targetPath, { relative }),
      relative ? 'Relative path copied.' : 'Path copied.'
    );
  }

  function handleRevealActiveFile() {
    const focusedTab = getFocusedTab();
    const path = focusedTab?.external ? null : focusedTab?.path;
    if (!path) {
      pushNotification('No workspace file is active to reveal.', 'warning');
      return;
    }

    setActivePanel('filepioneer');
    workspace.revealNode(path);
    refresh();
  }

  function handleSearchReplaceComplete({ files, replacements }) {
    refresh();
    pushNotification(
      `Replaced ${replacements} match${replacements === 1 ? '' : 'es'} across ${files} file${files === 1 ? '' : 's'}.`
    );
  }
  ...
  function buildSyncState() {
    ...
    nextState.backpackSnippets = backpackSnippets;
  ...
  function applySyncedState(nextState) {
    ...
    if (nextState.backpackSnippets && Array.isArray(nextState.backpackSnippets)) {
      handleBackpackSnippetsChange(nextState.backpackSnippets);
    }

    if (nextState.settings && typeof nextState.settings === 'object') {
      setSettings((current) => mergeWithDefaultSettings({
        ...current,
        ...nextState.settings,
      }));
    }
    ...
    if (nextState.layout && typeof nextState.layout === 'object') {
      ...
      setBottomPanelView(nextState.layout.bottomPanelView || 'terminal');
      setTerminalOpen(Boolean(nextState.layout.terminalOpen));
      setTerminalHeight(Number(nextState.layout.terminalHeight) || 220);
      setTerminalMaximized(Boolean(nextState.layout.terminalMaximized));
      ...
      authPopupPollerRef.current = window.setInterval(async () => {
        try {
          ...
          if (result?.status === 'complete' && result.session) {
            const sId = result.session?.sessionId || result.session?.id;
            if (sId) {
              localStorage.setItem('tilder_session_id', sId);
            }
            const session = {
  ...
  async function handleDisconnectProvider(provider) {
    try {
      ...
      localStorage.removeItem('tilder_session_id');
      hydratedSyncUserKeyRef.current = '';
      setAuthSession({
        ...DEFAULT_AUTH_SESSION,
        ...session,
    ...
    return Object.entries(debugBreakpoints).flatMap(([path, lines]) => {
      ...
      return [...lines]
        ...
        .map((line) => ({
          ...
          condition: breakpointConditions[`${path}:${line}`] || '',
        }));
    });
  }
  ...
  const debugExecutionDecorationsRef = useRef([]);

  function refreshDebugExecutionDecorations(
    editorInstance = getPreferredEditor(),
    monacoInstance = getPreferredMonaco(),
    pausedLine = null,
    variables = []
    ...
    if (pausedLine) {
      newDecorations.push({
        ...
        options: {
          ...
          glyphMarginClassName: 'debug-execution-glyph',
        },
      });

      // Show top variables as inline values
    ...
    debugExecutionDecorationsRef.current = editor.deltaDecorations(
      debugExecutionDecorationsRef.current,
      newDecorations
    );
  }
  ...
  async function toggleBreakpoint(path, line) {
    ...
    const existing = new Set(debugBreakpoints[path] || []);
    const removed = existing.has(line);
    if (removed) {
      existing.delete(line);
      if (debugSession.status !== 'idle') {
        await debugApi.removeBreakpoint(path, line - 1).catch(() => {});
      }
      setBreakpointConditions((conditions) => {
        if (!conditions[`${path}:${line}`]) {
          return conditions;
        }

        const nextConditions = { ...conditions };
        delete nextConditions[`${path}:${line}`];
        return nextConditions;
      });
    } else {
      existing.add(line);
      if (debugSession.status !== 'idle') {
        const condition = breakpointConditions[`${path}:${line}`] || '';
        await debugApi.setBreakpoint(path, line - 1, condition).catch(() => {});
      }
    }
  ...
  function handleSetBreakpointCondition(breakpoint) {
    if (!breakpoint?.path || !breakpoint?.line) {
      return;
    }

    const key = `${breakpoint.path}:${breakpoint.line}`;
    requestPrompt({
      title: 'Breakpoint Condition',
      message: `Set a condition for ${breakpoint.name} on line ${breakpoint.line}.`,
      defaultValue: breakpointConditions[key] || '',
      placeholder: 'Example: count > 5',
      confirmLabel: 'Save',
    }).then((nextValue) => {
      if (nextValue == null) {
        return;
  ...
  function resolveConfirmation(result, remember = false) {
    if (result && remember && confirmationRequest?.onRemember) {
      confirmationRequest.onRemember();
    }
  ...
  function toggleSystemMonitor(nextState) {
    if (typeof nextState === 'boolean') {
      if (nextState) {
        setModalType('System Monitor');
        setModalOpen(true);
      } else if (modalType === 'System Monitor') {
        closeCurrentModal();
      }
      return;
    }

    if (modalOpen && modalType === 'System Monitor') {
      closeCurrentModal();
      return;
    }
  ...
  function toggleStatusBarVisibility() {
    setShowStatusBar((current) => !current);
  }
  ...
  function toggleCenteredLayout() {
    setCenteredLayout((current) => !current);
  }
  ...
  function recordRecentFile(entry) {
    ...
    const recentEntry = {
      ...
      name: entry.name || entry.path || entry.id,
    ...
    setRecentFiles((current) => [
      recentEntry,
      ...current.filter((item) => item.path !== recentEntry.path),
    ].slice(0, 12));
  }

  function recordRecentWorkspace(entry) {
    if (!entry?.name) {
      return;
    }
    ...
    setRecentWorkspaces((current) => [
      recentEntry,
      ...current.filter((item) => item.id !== recentEntry.id),
    ].slice(0, 8));
  }

  async function handleOpenRecentFile(entry) {
    if (!entry?.path) {
      return;
    }

    const existingTab = workspace.tabs.find((tab) => tab.path === entry.path || tab.id === entry.path);
    if (existingTab) {
      workspace.setActiveTab(existingTab.id);
      refresh();
      pushNotification(`Reopened recent file ${existingTab.name}.`);
      return;
    }

    const node = workspace.findNode(entry.path);
    if (node) {
      await handleOpenNode(node, { preview: false });
      pushNotification(`Opened recent file ${node.name}.`);
      return;
    }

    pushNotification(`Recent file ${entry.name || entry.path} is not available in the current workspace.`);
  }

  async function handleOpenRecentWorkspace(entry) {
    if (!entry?.name) {
      return;
    }

    pushNotification(`Pick the folder for ${entry.name} to reopen it in the browser.`);
    await handleOpenFolder();
  }

  function escapeCurrentMode() {
    if (zenMode) {
      toggleZenMode();
      return;
    }
  ...
  escapeCurrentModeRef.current = escapeCurrentMode;
  ...
  function toggleZenMode() {
    ...
    setZenMode(false);
    setActivePanel(restore.activePanel ?? firstAvailableSidebarPanel() ?? DEFAULT_SIDEBAR_PANEL);
    setTerminalOpen(Boolean(restore.terminalOpen));
    setInfoDisplay(restore.infoDisplay ?? 'none');
    setShowMenuBar(restore.showMenuBar ?? true);
    setShowStatusBar(restore.showStatusBar ?? true);
    setCenteredLayout(Boolean(restore.centeredLayout));
    setLivePreviewOpen(Boolean(restore.livePreviewOpen));
  }
  ...
  function openBottomPanel(view = 'terminal') {
  ...
  function appendOutputLines(lines = [], source = 'Tilder') {
    ...
    setOutputEntries((current) => [
      ...current,
      {
        ...
        source,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        lines: normalizedLines.map((line) => String(line)),
      },
    ].slice(-80));
  }

  function appendDebugConsoleLines(lines = [], source = 'Debug') {
    const normalizedLines = Array.isArray(lines) ? lines : [lines];
    setDebugConsoleEntries((current) => [
      ...current,
      {
  ...
  async function runCodeWithCodeRunner(tabArg) {
    const tab = tabArg || getFocusedTab() || workspace.getActiveTab();
    if (!tab) {
      return;
    }
    ...
    try {
      ...
      appendOutputLines([`Running ${tab.name}...`], 'Code Runner');
      setTimeout(() => {
      ...
      if (localResult.supported) {
        const outputLines = formatLocalRunResult(localResult);
        appendOutputLines(outputLines, 'Code Runner');
        setTimeout(() => {
          terminalApiRef.current?.writeLines?.(outputLines);
      ...
      if (!language) {
        ...
        appendOutputLines([`No runner is configured for ${tab.language || 'this file type'}.`], 'Code Runner');
        setTimeout(() => {
      ...
      appendOutputLines([`Falling back to remote runner: ${language.name}`], 'Code Runner');
      setTimeout(() => {
      ...
      const outputLines = formatRunResult(result);
      appendOutputLines(outputLines, 'Code Runner');
      setTimeout(() => {
        terminalApiRef.current?.writeLines?.(outputLines);
        terminalApiRef.current?.focus?.();
      ...
      openBottomPanel('terminal');
      appendOutputLines([`Runner error: ${error instanceof Error ? error.message : 'Unknown error'}`], 'Code Runner');
      setTimeout(() => {
  ...
  async function handleCloneGitHubRepo(repoFullName) {
    if (!isDesktopRuntime()) {
      pushNotification('Cloning is only supported in the desktop app.', 'warning');
      return;
    }
    
    const targetFolder = await desktopPickFolder();
    if (!targetFolder) {
      return;
    }
    ...
    const cmd = isWindows
      ? `cd "${targetFolder}"; git clone https://github.com/${repoFullName}.git`
      : `cd "${targetFolder}" && git clone https://github.com/${repoFullName}.git`;

    openBottomPanel('terminal');
    setTerminalOpen(true);
    pushNotification(`Cloning ${repoFullName} into ${targetFolder}...`, 'info');

    if (terminalApiRef.current && terminalApiRef.current.createSession) {
      terminalApiRef.current.createSession({
        title: `Clone: ${repoName}`,
        executeInitialCommand: cmd
      });
      pushNotification(`Once cloning finishes, open the ${repoName} folder using Open Folder!`, 'info');
    } else {
      pushNotification('Terminal is not ready to execute clone command.', 'warning');
    }
  }
  ...
  useEffect(() => {
    return () => {
      ...
      lspBridgeCacheRef.current.forEach((bridge) => bridge?.dispose?.());
      lspBridgeCacheRef.current.clear();
    };
  }, []);
  ...
  const breadcrumbsEnabled = settings.breadcrumbs?.enabled !== false;
  ...
  const sidebarPanelWidth = useMemo(() => (hasSidebar ? sidebarWidth : 72), [hasSidebar, sidebarWidth]);
  const editorContentWidth = useMemo(() => {
    const baseWidth = hasSidebar ? `calc(100vw - ${sidebarPanelWidth}px)` : '100vw';
    return previewColumnOpen ? `calc(${baseWidth} - ${livePreviewWidth + 10}px)` : baseWidth;
  }, [livePreviewWidth, previewColumnOpen, hasSidebar, sidebarPanelWidth]);
  ...
  const commandPaletteCommands = useMemo(
    () => {
      ...
      const extensionCatalogKeys = Object.keys(extensionCatalog || {});
      for (const extId of extensionCatalogKeys) {
        const ext = extensionCatalog[extId];
        const extState = extensionState[extId];
        if (extState && extState.enabled && ext.manifest?.contributes?.commands) {
          for (const cmd of ext.manifest.contributes.commands) {
            merged.set(cmd.command, {
              id: cmd.command,
              label: cmd.title,
              category: ext.manifest.displayName || ext.manifest.name,
              keywords: [ext.manifest.name, 'extension'],
              bindingLabel: '',
            });
          }
        }
    ...
    [activeEditorPane, activeTab?.id, commandPaletteEditorNonce, effectiveBindings, focusedSplitPaneIndex, focusedSplitTab?.id, primaryActiveTabId, splitEditorOpen, version, extensionCatalog, extensionState]
  );
  ...
  function getPreferredEditor() {
    const editor = activeEditorPaneRef.current === 'secondary'
      ? splitEditorRefs.current.get(focusedSplitPaneIndex) || null
      : editorRef.current;
    window.getPreferredEditor = () => editor;
    return editor;
  }
  ...
  function getMountedEditorForTab(tabId) {
    if (!tabId) {
      return null;
    }

    const primaryTabId = splitEditorOpen ? primaryActiveTabId : workspace.activeTabId;
    if (primaryTabId === tabId) {
      return editorRef.current;
    }

    for (const [paneIndex, group] of splitPaneGroups.entries()) {
      if (group?.activeTabId === tabId) {
        return splitEditorRefs.current.get(paneIndex) || null;
      }
    }
  ...
  async function readWorkspacePathContent(path) {
    const openedTab = workspace.tabs.find((entry) => entry.path === path || entry.id === path);
    if (openedTab?.content != null) {
      return openedTab.content;
    }
    ...
    try {
      return (await workspace.readFile(node)).content || '';
    } catch {
      return '';
    }
  }

  function getLinePreviewFromContent(content, lineNumber) {
    const lines = String(content || '').split(/\r?\n/);
    return (lines[Math.max(0, Number(lineNumber || 1) - 1)] || '').trim();
  }

  function getCurrentSymbolQuery() {
    const editor = getPreferredEditor();
    const model = editor?.getModel?.();
    const position = editor?.getPosition?.();
    const selection = editor?.getSelection?.();
    const selectedText = model && selection && !selection.isEmpty() ? model.getValueInRange(selection).trim() : '';
  ...
  function decodeWorkspaceRelativePathFromUri(uri) {
    if (!uri) {
      return '';
    }
    ...
    try {
      ...
      const workspaceRoot = String(editorWorkspaceSession?.workspaceRoot || '')
        .replace(/\\/g, '/')
        .replace(/^\/([A-Za-z]:)/, '$1')
        .replace(/\/+$/, '');
  ...
  function getCurrentEditorLspContext() {
    const tab = getFocusedTab();
    const editor = getPreferredEditor();
    if (!tab || !editor || !activeLspBridge) {
      return null;
    }
    ...
    return {
      ...
      text: editor.getValue?.() ?? tab.content ?? '',
      relativePath: tab.path === 'root' ? tab.name : tab.path,
      fileName: tab.name,
    };
  }

  function normalizeLspLocations(response) {
    ...
    return rawLocations
      .map((entry) => {
        ...
        if (entry.targetUri) {
          return {
            uri: entry.targetUri,
            range: entry.targetSelectionRange || entry.targetRange || null,
          };
        }
  ...
  async function buildNavigationResults(response, fallbackName) {
    const seen = new Set();
    const results = [];

    for (const location of normalizeLspLocations(response)) {
      const relativePath = decodeWorkspaceRelativePathFromUri(location.uri);
      if (!relativePath) {
        continue;
      }
      ...
      const cacheKey = `${relativePath}:${line}:${column}`;
      if (seen.has(cacheKey)) {
        continue;
      }
      seen.add(cacheKey);

      const node = workspace.findNode(relativePath);
      const previewContent = await readWorkspacePathContent(relativePath);
      results.push({
        kind: 'symbol',
        path: relativePath,
        name: node?.name || fallbackName || relativePath.split('/').pop() || relativePath,
        symbolType: fallbackName ? 'symbol' : '',
        line,
        column,
        preview: getLinePreviewFromContent(previewContent, line),
      });
    }
  ...
  function buildTextEditOffset(content, position) {
    const normalized = String(content || '').replace(/\r\n/g, '\n');
    const lines = normalized.split('\n');
    const lineIndex = Math.max(0, Math.min(Number(position?.line || 0), lines.length - 1));
    ...
    return offset + Math.min(character, lines[lineIndex]?.length || 0);
  }

  async function applyTextEditsToPath(path, edits = []) {
    if (!path || !edits.length) {
      return false;
    }

    const previousActiveTabId = workspace.activeTabId;
    const previousFocusedPane = activeEditorPaneRef.current;
    const node = workspace.findNode(path);
    if (!node || node.type !== 'file') {
      return false;
    }

    const existingTab = workspace.tabs.find((entry) => entry.path === path);
    if (!existingTab) {
    ...
    const normalizedCurrent = String(tab.content ?? '').replace(/\r\n/g, '\n');
    const nextContent = [...edits]
      .sort((left, right) => {
        const rightOffset = buildTextEditOffset(normalizedCurrent, right.range?.start);
        const leftOffset = buildTextEditOffset(normalizedCurrent, left.range?.start);
        return rightOffset - leftOffset;
      })
      .reduce((content, edit) => {
        const startOffset = buildTextEditOffset(content, edit.range?.start);
        const endOffset = buildTextEditOffset(content, edit.range?.end);
        return `${content.slice(0, startOffset)}${edit.newText || ''}${content.slice(endOffset)}`;
      }, normalizedCurrent);

    workspace.updateTabContent(tab.id, nextContent);

    if (previousActiveTabId && workspace.tabs.some((entry) => entry.id === previousActiveTabId)) {
      workspace.setActiveTab(previousActiveTabId);
    }

    if (previousFocusedPane === 'secondary') {
  ...
  async function applyLspWorkspaceEdit(workspaceEdit) {
    const groupedEdits = new Map();
    const changeEntries = workspaceEdit?.changes && typeof workspaceEdit.changes === 'object'
      ? Object.entries(workspaceEdit.changes)
      : [];

    changeEntries.forEach(([uri, edits]) => {
      const relativePath = decodeWorkspaceRelativePathFromUri(uri);
      if (!relativePath || !Array.isArray(edits) || !edits.length) {
        return;
      }
      groupedEdits.set(relativePath, edits);
    });

    (workspaceEdit?.documentChanges || []).forEach((change) => {
      const uri = change?.textDocument?.uri;
      const relativePath = decodeWorkspaceRelativePathFromUri(uri);
      if (!relativePath || !Array.isArray(change?.edits) || !change.edits.length) {
        return;
      }
      const existing = groupedEdits.get(relativePath) || [];
      groupedEdits.set(relativePath, [...existing, ...change.edits]);
    });

    let appliedCount = 0;
    for (const [path, edits] of groupedEdits.entries()) {
      if (await applyTextEditsToPath(path, edits)) {
        appliedCount += 1;
      }
    }
  ...
  useEffect(() => {
    const noticeKey = `${intelliSenseTargetTab?.language || ''}:${activeIntelliSense?.statusLabel || ''}:${activeIntelliSense?.detail || ''}`;
    if (
      !intelliSenseTargetTab?.language ||
      !activeIntelliSense ||
  ...
  }, [activeIntelliSense, intelliSenseTargetTab?.language]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      intelliSenseTargetTab?.id,
      intelliSenseTargetTab?.language,
      intelliSenseTargetTab?.name,
      intelliSenseTargetTab?.path,
      editorCapabilitiesStatus,
  ...
  useEffect(() => {
    ...
    let bridge = lspBridgeCacheRef.current.get(bridgeKey);
    if (!bridge) {
      activeLspBridgeKeyRef.current = bridgeKey;
      bridge = createLspBridge({
        languageId: intelliSenseTargetTab.language,
        sessionId: editorWorkspaceSession.sessionId,
        workspaceRoot: editorWorkspaceSession.workspaceRoot,
        onStatus: (payload) => {
          if (activeLspBridgeKeyRef.current !== bridgeKey) {
            return;
          }

          setLspBridgeState({
            status: payload?.status || 'unknown',
            languageId: payload?.languageId || intelliSenseTargetTab.language,
            message: payload?.message || payload?.serverLabel || '',
          });
        },
      });
      lspBridgeCacheRef.current.set(bridgeKey, bridge);
    } else {
      activeLspBridgeKeyRef.current = bridgeKey;
      setLspBridgeState({
        status: 'connected',
        languageId: intelliSenseTargetTab.language,
        message: 'Local language server ready',
      });
    }
    ...
    return () => {
      if (lspBridgeRef.current === bridge && activeLspBridgeKeyRef.current === bridgeKey) {
        lspBridgeRef.current = null;
        activeLspBridgeKeyRef.current = '';
      }
  ...
  useEffect(() => {
    ...
    const relativePath =
      intelliSenseTargetTab.path === 'root' ? intelliSenseTargetTab.name : intelliSenseTargetTab.path;
    const timer = window.setTimeout(() => {
      const warmBridge =
        typeof activeLspBridge.ensureInitialized === 'function'
          ? activeLspBridge.ensureInitialized()
          : Promise.resolve();

      warmBridge
        .then(() =>
          activeLspBridge.syncDocument({
            relativePath,
            fileName: intelliSenseTargetTab.name,
            text: intelliSenseTargetTab.content ?? '',
          })
        )
        .catch(() => null);
    }, 60);
  ...
  const getEditorIntelliSense = useCallback(
    (tab) => {
      if (!tab?.id || tab.id !== intelliSenseTargetTab?.id) {
        return null;
      }
      return activeIntelliSense;
    },
    [activeIntelliSense, intelliSenseTargetTab?.id]
  );

  const getEditorLspBridge = useCallback(
    (tab) => {
      if (!tab?.id || tab.id !== intelliSenseTargetTab?.id) {
        return null;
      }
      return activeIntelliSenseProviderType === 'lsp' && activeIntelliSenseAvailable ? activeLspBridge : null;
    },
    [activeIntelliSenseAvailable, activeIntelliSenseProviderType, activeLspBridge, intelliSenseTargetTab?.id]
  );
  ...
  function queueSave(tabId) {
    const tab = workspace.tabs.find((entry) => entry.id === tabId);
    if (!tabId || (!tab?.handle && !tab?.nativePath) || tab.isUntitled) {
      return;
    }
  ...
  async function findLocalDefinitionResults(symbolName) {
    const query = String(symbolName || '').trim();
    if (!query) {
      return [];
    }

    const pool = getSearchPool(workspace, 'workspace').filter((entry) =>
      matchesPathFilters(entry.path, '', 'node_modules, dist')
    );
    const preferredPath = getFocusedTab()?.path || '';
    const results = [];

    for (const entry of pool.sort((left, right) => {
      if (left.path === preferredPath) return -1;
      if (right.path === preferredPath) return 1;
      ...
      const match = collectSymbols(content).find((symbol) => symbol.name === query);
      if (!match) {
        continue;
      }
  ...
  async function findLocalReferenceResults(symbolName) {
    const query = String(symbolName || '').trim();
    if (!query) {
      return [];
    }
    ...
    for (const entry of pool) {
      ...
      let match;
      while ((match = matcher.exec(content)) && results.length < 40) {
        ...
        results.push({
          ...
          preview: getLinePreviewFromContent(content, line),
        });
      }
  ...
  function openNavigationOverlay(mode, value, results, error = '') {
    setGoOverlay({
      open: true,
      mode,
      value,
  ...
  async function fetchDefinitionResults() {
    const query = getCurrentSymbolQuery();
    const lspContext = getCurrentEditorLspContext();

    if (lspContext) {
      try {
        const response = await activeLspBridge.requestDefinition({
          relativePath: lspContext.relativePath,
          fileName: lspContext.fileName,
          text: lspContext.text,
          position: lspContext.position,
        });
        const results = await buildNavigationResults(response, query);
        if (results.length) {
          return { query, results };
        }
      } catch {
        // Fall back to workspace symbol search when LSP navigation fails.
      }
    }

    return {
      query,
      results: await findLocalDefinitionResults(query),
    };
  }

  async function fetchReferenceResults() {
    const query = getCurrentSymbolQuery();
    const lspContext = getCurrentEditorLspContext();

    if (lspContext) {
      try {
        const response = await activeLspBridge.requestReferences({
          relativePath: lspContext.relativePath,
          fileName: lspContext.fileName,
          text: lspContext.text,
          position: lspContext.position,
        ...
        // Fall back to workspace text matches when LSP references fail.
      }
    }

    return {
      query,
      results: await findLocalReferenceResults(query),
    };
  }

  async function formatTabBeforeSave(tabId) {
    if (!settings.formatOnSave) {
      return false;
    }

    const tab = workspace.tabs.find((entry) => entry.id === tabId);
    const editor = getMountedEditorForTab(tabId);
    if (!tab || !editor?.getAction) {
      return false;
    }
    ...
    try {
      editor.focus();
      await action.run();
      workspace.updateTabContent(tab.id, editor.getValue?.() ?? tab.content ?? '');
      refresh();
      return true;
    } catch {
      return false;
  ...
  async function handleGoToDefinition() {
    ...
    if (results.length === 1) {
      await handleOpenSearchResult(results[0], { preview: false });
      return;
    }
  ...
  async function handleGoToReferences() {
    ...
    const { query, results } = await fetchReferenceResults();
    if (!results.length) {
      pushNotification(`No references found for ${query || 'the current symbol'}.`, 'warning');
      return;
    }

    openNavigationOverlay('reference', query, results);
  }
  ...
  async function handleRenameSymbol() {
    const query = getCurrentSymbolQuery();
    if (!query) {
      pushNotification('Place the cursor on a symbol first.', 'warning');
      return;
    }

    const nextName = await requestPrompt({
      title: 'Rename Symbol',
      message: `Rename "${query}" to:`,
      defaultValue: query,
      placeholder: 'Enter the new symbol name',
      confirmLabel: 'Rename',
    });
    if (nextName == null) {
      return;
    ...
    if (!trimmedName || trimmedName === query) {
      return;
    }
    ...
    if (lspContext) {
      try {
        const workspaceEdit = await activeLspBridge.requestRename({
          relativePath: lspContext.relativePath,
          fileName: lspContext.fileName,
          text: lspContext.text,
          position: lspContext.position,
        ...
        const appliedCount = await applyLspWorkspaceEdit(workspaceEdit);
        if (appliedCount) {
          pushNotification(`Renamed ${query} to ${trimmedName}.`);
          return;
        }
      } catch {
        // Fall back to a local single-file rename below.
      }
    }
    ...
    const editor = getPreferredEditor();
    const currentContent = editor?.getValue?.() ?? tab.content ?? '';
    const matcher = buildMatcher(query, {
      caseSensitive: true,
      wholeWord: true,
      useRegex: false,
    ...
    const nextContent = currentContent.replace(matcher, trimmedName);
    workspace.updateTabContent(tab.id, nextContent);
    refresh();
    pushNotification(`Renamed ${query} to ${trimmedName} in ${tab.name}.`);
  }
  ...
  function handleGoToOverlayResultSelect(result) {
  ...
  function submitGoOverlayAction() {
    ...
    if (goOverlay.results.length) {
      handleGoToOverlayResultSelect(goOverlay.results[0]);
    }
  }
  ...
  function handleSetLanguage(language) {
    ...
    // Auto-rename untitled file to match the selected language's extension
    if (tab.isUntitled) {
      ...
      const ext = LANGUAGE_TO_EXTENSION[language.toLowerCase()] || 'txt';
      const lastDot = tab.name.lastIndexOf('.');
      const base = lastDot !== -1 ? tab.name.substring(0, lastDot) : tab.name;
      tab.name = `${base}.${ext}`;
    }
  ...
  async function handleOpenFolder() {
    try {
      ...
      recordRecentWorkspace({
        id: workspace.rootName || 'workspace',
        name: workspace.rootName || workspace.getRootNode()?.name || 'Workspace',
      });
      ...
      if (!trustedWorkspaces[rootPath]) {
        workspace.isTrusted = false;
        setPendingTrustFolder({ name: workspace.rootName || 'Workspace', path: rootPath });
      } else {
  ...
  async function handleAddFolderToWorkspace() {
    await workspace.addFolderToWorkspace();
    workspace.setSelectedNode('root');
    if (builtInFeatures.explorer) {
      setActivePanel('filepioneer');
    }
  ...
  async function handleSaveWorkspace() {
    ...
    if (path) {
      pushNotification(`Workspace saved to ${path}`, 'success');
  ...
  async function handleOpenFileDialog() {
    try {
      ...
      if (openedTab?.id) {
        ...
        recordRecentFile(openedTab);
      }
      pushNotification(`Opened file ${openedTab?.name || 'file'}.`);
      refresh();
    } catch (err) {
      if (err?.message && !err.message.toLowerCase().includes('user abort') && !err.message.toLowerCase().includes('cancelled')) {
        pushNotification(err.message || 'Could not open file.', 'error');
      }
    }
  }
  ...
  async function handleOpenSearchResult(result, openOptions = {}) {
    const preview = openOptions.preview !== false;
    const existingTab = workspace.tabs.find((tab) => tab.path === result.path || tab.id === result.path);

    if (existingTab) {
      workspace.setActiveTab(existingTab.id);
      assignTabToFocusedGroup(existingTab.id, { preview });
    } else {
  ...
  async function handleCreateUntitledFile() {
    const parentPath = getCreateParentPath();
    const fileName = await requestPrompt({
      title: 'New File',
      message: 'Enter file name with extension (e.g. main.cpp, App.java). A template will be applied automatically:',
    ...
    if (!fileName) {
      // User cancelled — just return, don't bump any nonces
      return;
    }
    ...
    workspace.createUntitledFile(parentPath, fileName, content);
    const tab = workspace.getActiveTab();
    if (tab) {
      assignTabToFocusedGroup(tab.id, { preview: false });
    }
    setActivePanel('filepioneer');
    refresh();
  }
  ...
  const handleNewWindow = async () => {
    if (isDesktopRuntime()) {
      try {
        ...
        pushNotification(err?.message || 'Could not open a new window.', 'warning');
      }
    } else {
  ...
  async function handleCheckForUpdates(isManual = false) {
    if (!isDesktopRuntime()) {
      if (isManual) {
        pushNotification('Updates can only be checked in the desktop app.', 'warning');
      }
      return;
    }

    if (isManual) {
      pushNotification('Checking for updates...');
    }

    try {
      // Lazy load to avoid import issues if plugin isn't there
      ...
      if (update?.available) {
        ...
        if (isManual) {
          pushNotification('Tilder is up to date!', 'info');
        }
      }
    } catch (err) {
      if (isManual) {
        pushNotification(`Failed to check for updates: ${err.message || err}`, 'error');
      }
    }
  }
  ...
  async function handleCreateFolder() {
    ...
    if (workspace.tree.length > 0) {
      ...
      pushNotification('Cannot create a folder in an empty workspace. Please save a file first or open a folder.', 'warning');
    }
    refresh();
  }

  function handleDeleteRequest(node) {
    if (settings?.explorer?.confirmDelete === false) {
      return true;
    }
    ...
    return requestConfirmation({
      ...
      onRemember: () => {
        updateSetting('explorer.confirmDelete', false);
      }
    });
  }
  ...
  async function handleSaveActiveFile() {
    ...
    try {
      await formatTabBeforeSave(tab.id);
  ...
  async function handleSaveWorkspaceAs() {
    try {
      ...
      if (err?.message && !err.message.toLowerCase().includes('user abort') && !err.message.toLowerCase().includes('cancelled')) {
        pushNotification(err.message || 'Could not save workspace.', 'error');
      }
    }
  }
  ...
  function activateNavigationTab(tabId) {
    const tab = workspace.tabs.find((entry) => entry.id === tabId || entry.path === tabId);
    if (!tab) {
      return false;
    }

    navigationHistoryRef.current.suppress = true;
    workspace.setActiveTab(tab.id);
    setActiveEditorPane('primary');
    assignTabToFocusedGroup(tab.id, { preview: false });
    setPrimaryActiveTabId(tab.id);
    touchGroupMru('primary', tab.id);
    refresh();
    return true;
  }
  ...
  function handleNavigateBack() {
    ...
    while (history.back.length) {
      const targetId = history.back.pop();
      if (!targetId || targetId === currentId) {
        continue;
      }

      if (currentId) {
        history.forward = [currentId, ...history.forward.filter((entry) => entry !== currentId)].slice(0, 100);
      }

      if (activateNavigationTab(targetId)) {
        history.current = targetId;
        return;
      }
    }

    pushNotification('No previous editor location.', 'info');
  }
  ...
  function handleNavigateForward() {
    ...
    while (history.forward.length) {
      const targetId = history.forward.shift();
      if (!targetId || targetId === currentId) {
        continue;
      }

      if (currentId) {
        history.back = [...history.back.filter((entry) => entry !== currentId), currentId].slice(-100);
      }
    ...
    pushNotification('No next editor location.', 'info');
  }
  ...
  useEffect(() => {
    ...
    const history = navigationHistoryRef.current;
    if (history.suppress) {
      history.current = currentId;
      history.suppress = false;
      return;
    }

    if (history.current && history.current !== currentId) {
      history.back = [...history.back.filter((entry) => entry !== history.current), history.current].slice(-100);
      history.forward = [];
    }

    history.current = currentId;
  }, [workspace.activeTabId, version]);
  ...
  async function handleDuplicateSelected() {
    ...
    const duplicated = await workspace.duplicateNode(targetPath);
    if (duplicated) {
      setActivePanel('filepioneer');
      pushNotification(`Duplicated ${duplicated.name}.`);
      refresh();
      return;
    }

    pushNotification('Could not duplicate that item.', 'warning');
  }

  function handleOpenInTerminal(path) {
    if (workspace.isTrusted === false) {
      pushNotification('Cannot open terminal in Restricted Mode. Please trust this workspace first.', 'error');
      return;
    }
    if (!builtInFeatures.terminal) {
      notifyExtensionDisabled('Integrated Terminal');
      return;
    }
    
    setTerminalOpen(true);
    setBottomPanelView('terminal');
    // Find parent directory if it's a file
    const node = workspace.findNode(path);
    const targetNode = node?.type === 'folder' ? node : workspace.findNode(node?.parentPath || 'root') || node;
    const nativeCwd = targetNode?.nativePath || null;
    
    if (nativeCwd) {
      terminalApiRef.current?.newSession?.({ initialCwd: nativeCwd });
    } else {
      terminalApiRef.current?.newSession?.({});
    }
  }
  ...
  async function handleStartDebugging() {
    ...
    let tab = getFocusedTab();
    if (!tab) {
      pushNotification('Open a file to start debugging.', 'warning');
      return;
    }

    // Ensure active file is saved before debugging. If it is untitled or dirty, save it.
    if ((tab.dirty || tab.isUntitled) && workspace.saveTab) {
      try {
        const saved = await workspace.saveTab(tab.id);
        if (!saved) {
          pushNotification('Please save the file first to debug it.', 'warning');
          return;
        }
        // Retrieve fresh tab reference since its path/name may have changed
        tab = getFocusedTab() || tab;
      } catch (err) {
        pushNotification('Please save the file first to debug it.', 'warning');
        return;
      }
    ...
    // Dynamic workspace mirroring & path resolution
    let targetPath = tab.nativePath || tab.path;
    try {
      const payload = await workspace.getSyncPayload?.();
      if (payload) {
        const syncRes = await syncTerminalWorkspaceRoot(payload);
        if (syncRes && syncRes.cwd) {
          if (tab.isUntitled) {
            targetPath = `${syncRes.cwd}/${tab.name}`.replace(/\\/g, '/');
          }
        }
      }
    } catch (err) {
      console.warn('Workspace sync failed, continuing debugging with local file path:', err);
    }

    if (!targetPath) {
      pushNotification('Could not resolve file path for debugging.', 'warning');
      return;
    }
    ...
    setDebugSession({
      status: 'starting',
      mode: 'Debug',
      message: `Initializing debugger for ${tab.name}...`,
    });
    
    openBottomPanel('debugConsole');

    try {
      // Start the actual debug session via CDP
      const { port, pid } = await debugApi.start(targetPath);
      
      setDebugSession({
        status: 'running',
        mode: 'Debug',
        message: `Debugging ${tab.name}`,
        port,
        pid,
        runtime: debugApi.runtime
      });

      if (!debugApi.isCDPSupported()) {
        pushNotification(`Started ${debugApi.runtime} process. Note: Interactive debugging (stepping, variables) is currently optimized for Node.js.`, 'info');
      }
      
      // Sync initial breakpoints
      if (debugApi.isCDPSupported()) {
        const currentBreakpoints = getActiveBreakpoints();
        for (const bp of currentBreakpoints) {
          await debugApi.setBreakpoint(bp.path, bp.line - 1, bp.condition);
        }
        // Automatically resume if needed (Node.js starts in paused state with --inspect-brk)
        await debugApi.resume();
      }
    } catch (error) {
      setDebugSession({
        status: 'idle',
        mode: 'Debug',
        message: `Debug failed: ${error.message || 'Check connection'}`,
      });
      pushNotification(`Debugger failed: ${error.message}`, 'error');
    }
  }

  async function handleEvaluateDebug(expression) {
    if (!debugApi.connected) return;
    ...
    const { result, error, wasThrown } = await debugApi.evaluate(expression);
    ...
    setDebugConsoleEntries((current) => [
      ...current,
      {
        ...
        source: 'Debug Console',
        time: new Date().toLocaleTimeString(),
        lines: [wasThrown ? (error || 'Evaluation error') : (result ?? 'undefined')],
        type: wasThrown ? 'error' : 'result'
      }
    ]);
  }

  const handleInsertSnippetIntoEditor = useCallback((code) => {
    const editor = getPreferredEditor();
    if (!editor) {
      pushNotification('Please open a file in the editor first.', 'info');
      return;
    }

    editor.focus();
    const selection = editor.getSelection();
    const monacoInstance = window.monaco || monacoRef.current;
    ...
    if (monacoInstance) {
      ...
      editor.executeEdits("backpack-insert", [op]);
      pushNotification('Snippet inserted.', 'success');
    } else {
      // Fallback if Monaco is not fully ready
      const model = editor.getModel();
      if (model) {
  ...
  function handleRunWithoutDebugging() {
    ...
    setDebugSession({
      status: 'running',
      mode: 'Run',
      message: tab ? `Running ${tab.name} without debugger` : 'Open a file to run.',
    });
  ...
  async function handleRunCurrentFile(mode = 'run_code') {
    let tab = getFocusedTab();
    if (!tab) return;
    
    if (workspace.isTrusted === false) {
      pushNotification('Cannot run code in Restricted Mode. Please trust this workspace first.', 'error');
      return;
    }

    if (mode === 'run_sandbox') {
      if (!isDesktopRuntime()) {
        pushNotification('Running .exe files in Windows Sandbox requires the Tilder desktop app.', 'warning');
        return;
      }
      ...
      try {
        const payload = await workspace.getSyncPayload?.();
        if (payload) {
          syncRes = await syncTerminalWorkspaceRoot(payload);
        }
      } catch (err) {
        console.warn('Workspace sync failed prior to sandbox runner:', err);
      }

      const workspaceCwd = syncRes?.cwd;
      if (!workspaceCwd) {
        pushNotification('Could not resolve workspace path for Windows Sandbox.', 'warning');
        return;
      }

      // If the file is dirty or untitled, prompt user to save first because Sandbox needs the physical file
      if (tab.dirty || tab.isUntitled) {
        try {
          const saved = await workspace.saveTab(tab.id);
          if (!saved) {
            pushNotification('Please save the file first to run it in Sandbox.', 'warning');
            return;
          }
          tab = getFocusedTab() || tab;
        } catch (err) {
      ...
      const filePath = tab.nativePath || (tab.path ? `${workspaceCwd}/${tab.path === 'root' ? tab.name : tab.path}`.replace(/\\/g, '/') : '');
      if (!filePath) {
        pushNotification('Could not resolve file path for Windows Sandbox.', 'warning');
        return;
      }

      openBottomPanel('terminal');
      setTerminalOpen(true);
      pushNotification('Preparing Sandbox execution and scanner...', 'info');

      try {
        const { prepareSandboxRun } = await import('./core/SandboxRunner.js');
        const { ps1Path } = await prepareSandboxRun(filePath, workspaceCwd);

        if (terminalApiRef.current && terminalApiRef.current.executeCommand) {
          // Execute the PowerShell script in a dedicated Sandbox run session
          const cmd = `powershell -ExecutionPolicy Bypass -NoExit -File "${ps1Path.replace(/\//g, '\\')}"`;
          if (terminalApiRef.current.createSession) {
            terminalApiRef.current.createSession({
              title: `Sandbox: ${tab.name}`,
              executeInitialCommand: cmd
            });
          } else {
            terminalApiRef.current.executeCommand(cmd);
          ...
          pushNotification('Terminal is not ready to execute Sandbox command.', 'warning');
        }
      } catch (err) {
        pushNotification(`Failed to initialize Sandbox execution: ${err.message}`, 'error');
      }
      return;
    }

    if (mode === 'debug_file' || mode === 'debug_launch') {
      handleStartDebugging();
      return;
    }

    // In desktop mode, 'run_code' always goes through the terminal (TempCodeRunner).
    // Only fall back to the cloud runner on web/non-desktop.
    if (mode === 'run_code' && !isDesktopRuntime()) {
      runCodeWithCodeRunner(tab);
      return;
    }
    ...
    if (mode === 'run_code' && (tab.dirty || tab.isUntitled) && isDesktopRuntime()) {
      isTempRun = true;
      try {
        const payload = await workspace.getSyncPayload?.();
        let root = payload?.cwd || '';
        if (root) {
          const tempDir = `${root}/.tilder/temp`.replace(/\\/g, '/');
          const tempName = tab.isUntitled ? (tab.name || 'untitled.txt') : `temp_${tab.name}`;
          const tempPath = `${tempDir}/${tempName}`;
          
          const { desktopWriteFile, desktopCreateFolder } = await import('./core/desktopFileApi.js');
          await desktopCreateFolder(tempDir).catch(() => {});
          await desktopWriteFile(tempPath, tab.content || '');
          
          tab = { ...tab, name: `.tilder/temp/${tempName}`, isUntitled: true };
        }
      } catch (err) {
        console.error('Failed to create temp runner file:', err);
      }
    } else if ((tab.dirty || tab.isUntitled) && workspace.saveTab) {
      try {
        const saved = await workspace.saveTab(tab.id);
        if (!saved) {
          pushNotification('Please save the file first to run it.', 'warning');
          return;
        }
        // Retrieve fresh tab reference since its path/name may have changed
        tab = getFocusedTab() || tab;
    ...
    openBottomPanel('terminal');
    setTerminalOpen(true);
    pushNotification(`Compiling/Running ${tab.name}...`, 'info');

    try {
      // Sync workspace mirror and resolve target directory
      let cwd = null;
      try {
        const payload = await workspace.getSyncPayload?.();
        if (payload) {
          const syncRes = await syncTerminalWorkspaceRoot(payload);
          cwd = syncRes?.cwd || null;
        }
      } catch (err) {
        console.warn('Workspace sync failed, proceeding with legacy runner:', err);
      }

      const runtimes = await getDetectedRuntimes();
      const runCommand = getRunCommandForTab(tab, { ...runtimes, ...customRuntimes }, cwd);

      if (!runCommand) {
        pushNotification(`Could not determine execution command for ${tab.language || 'this file'}.`, 'warning');
        setDebugSession({
          status: 'idle',
          mode: 'Ready',
      ...
      // We have a terminal instance reference (terminalApiRef.current).
      // We can use it to execute the run command in the shell natively!
      if (terminalApiRef.current && terminalApiRef.current.executeCommand) {
        if (mode === 'run_terminal' && terminalApiRef.current.createSession) {
          terminalApiRef.current.createSession({ title: tab.name, executeInitialCommand: runCommand });
        } else {
          // Send the command directly into the user's terminal UI
          terminalApiRef.current.executeCommand(runCommand);
        }
      } else {
        pushNotification('Terminal is not ready to execute the command.', 'warning');
      }

      setDebugSession({
        status: 'idle',
        mode: 'Ready',
        message: 'Program executed in terminal.',
      });

    } catch (err) {
      pushNotification(`Run failed: ${err.message}`, 'error');
    }
  }

  async function handlePauseDebugging() {
    try {
      await debugApi.pause();
    } catch (error) {
      pushNotification(`Pause failed: ${error.message}`, 'warning');
    }
  }
  ...
  async function handleContinueDebugging() {
    try {
      ...
      pushNotification(`Continue failed: ${error.message}`, 'warning');
    }
  }

  async function handleStopDebugging() {
    try {
      await debugApi.stop();
      setDebugSession({
        status: 'idle',
      ...
      pushNotification(`Stop failed: ${error.message}`, 'warning');
    }
  }

  async function handleRestartDebugging() {
    await handleStopDebugging();
    setTimeout(() => handleStartDebugging(), 500);
  }
  ...
  const debugCallStack = useMemo(() => {
    const focusedTab = getFocusedTab();
    const activeEditor = getPreferredEditor();
    const currentContent = activeEditor?.getValue?.() ?? focusedTab?.content ?? '';
    const currentLine = editorStatus.line || 1;
    const symbolFrames = collectSymbols(currentContent)
      .filter((symbol) => symbol.line <= currentLine)
      .sort((left, right) => left.line - right.line)
      .slice(-3)
      .map((symbol) => ({
        id: `${focusedTab?.path || 'active'}:${symbol.line}:${symbol.name}`,
        label: symbol.name,
        location: `${focusedTab?.name || 'file'}:${symbol.line}`,
        kind: symbol.type,
        current: false,
      }));
    ...
    const rootFrame = {
      id: 'session-frame',
      label: debugSession.mode === 'Debug' ? 'Debug Session' : debugSession.mode === 'Run' ? 'Run Session' : 'Editor Session',
      location: focusedTab?.path || 'No active file',
      kind: debugSession.status,
      current: false,
    };

    const currentFrame = {
      id: 'current-frame',
      label: focusedTab?.name || 'No active file',
      location: focusedTab ? `${focusedTab.path}:${currentLine}` : 'No source loaded',
      kind: 'current',
      current: true,
    };

    return [rootFrame, ...symbolFrames, currentFrame];
  }, [debugSession.mode, debugSession.status, editorStatus.line, getFocusedTab, workspace.activeTabId, version]);

  const activeDebugStack = debugApi.connected && realDebugStack.length > 0 ? realDebugStack : debugCallStack;
  ...
  const debugVariables = useMemo(() => {
    ...
    return [
      { name: 'fileName', value: focusedTab?.name || 'n/a', scope: 'session' },
      { name: 'filePath', value: focusedTab?.path || 'n/a', scope: 'session' },
      { name: 'language', value: focusedTab?.language || 'plaintext', scope: 'session' },
      { name: 'line', value: String(editorStatus.line || 1), scope: 'cursor' },
      { name: 'column', value: String(editorStatus.column || 1), scope: 'cursor' },
      { name: 'selectionLength', value: String(editorStatus.selectionLength || 0), scope: 'cursor' },
      { name: 'selection', value: selection || '""', scope: 'cursor' },
      { name: 'breakpoints', value: String(getActiveBreakpoints().length), scope: 'debug' },
      { name: 'watchExpressions', value: String(watchExpressions.length), scope: 'debug' },
    ];
  }, [
    activeTab?.id,
    activeTab?.language,
    ...
    editorStatus.column,
    editorStatus.line,
    editorStatus.selectionLength,
    watchExpressions.length,
  ]);

  const activeDebugVariables = debugApi.connected && realDebugVariables.length > 0 ? realDebugVariables : debugVariables;
  ...
  useEffect(() => {
    if (editorRef.current && monacoRef.current && primaryActiveTabId) {
      refreshBreakpointDecorations(
        editorRef.current,
        monacoRef.current,
        primaryActiveTabId,
        breakpointDecorationsRef
      );
    }
  }, [debugBreakpoints, primaryActiveTabId]);
  ...
  const commandActions = {
    ...
    'workbench.action.escapeCurrentMode': () => escapeCurrentMode(),
    'workbench.action.toggleCenteredLayout': () => toggleCenteredLayout(),
    'workbench.action.toggleStatusbarVisibility': () => toggleStatusBarVisibility(),
    'workbench.action.toggleMenuBar': () => toggleMenuBarVisibility(),
    'breadcrumbs.toggle': () =>
      setSettings((current) => ({
        ...current,
        breadcrumbs: {
          ...(current.breadcrumbs || {}),
          enabled: current.breadcrumbs?.enabled === false,
        },
      })),
    'editor.action.toggleMinimap': () =>
      setSettings((current) => ({
        ...
        minimap: {
          ...(current.minimap || {}),
          enabled: !current.minimap?.enabled,
        },
      })),
    'workbench.actions.view.problems': () => openBottomPanel('problems'),
    'workbench.action.output.toggleOutput': () => openBottomPanel('output'),
    'workbench.debug.action.toggleRepl': () => openBottomPanel('debugConsole'),
    ...
    'workbench.action.terminal.split': () => {
      toggleTerminalPanel(true);
      terminalApiRef.current?.splitSession?.();
    },
    ...
    'workbench.action.navigateBack': () => handleNavigateBack(),
    'workbench.action.navigateForward': () => handleNavigateForward(),
    ...
    'workbench.action.selectTheme': () => setThemePickerOpen(true),
    'workbench.files.action.showActiveFileInExplorer': () => handleRevealActiveFile(),
    'explorer.copyPath': () => handleCopyExplorerPath(),
    'explorer.copyRelativePath': () => handleCopyExplorerPath({ relative: true }),
    'explorer.duplicate': () => handleDuplicateSelected(),
    'editor.action.revealDefinition': () => handleGoToDefinition(),
    'editor.action.goToReferences': () => handleGoToReferences(),
    'editor.action.peekDefinition': () => handlePeekDefinition(),
    'editor.action.peekReferences': () => handlePeekReferences(),
    'editor.action.rename': () => handleRenameSymbol(),
    ...
    'workbench.action.debug.stepOver': () => debugApi.stepOver().catch(() => {}),
    'workbench.action.debug.stepInto': () => debugApi.stepInto().catch(() => {}),
    'workbench.action.debug.stepOut': () => debugApi.stepOut().catch(() => {}),
    'workbench.action.debug.continue': () => handleContinueDebugging(),
    'workbench.action.debug.pause': () => handlePauseDebugging(),
    ...
    'workbench.action.newWindow': () => handleNewWindow(),
    'workbench.action.checkForUpdates': () => handleCheckForUpdates(true),
  };
  ...
  useEffect(() => {
    function handleDoubleEscape(event) {
      if (event.key !== 'Escape' || event.repeat || event.ctrlKey || event.metaKey || event.altKey || event.shiftKey) {
        return;
      }

      const now = Date.now();
      const previous = lastEscapePressRef.current;
      lastEscapePressRef.current = now;

      if (previous && now - previous <= 420) {
        lastEscapePressRef.current = 0;
        event.preventDefault();
        event.stopPropagation();
        escapeCurrentModeRef.current?.();
      }
    }

    window.addEventListener('keydown', handleDoubleEscape, true);
    return () => {
      window.removeEventListener('keydown', handleDoubleEscape, true);
    };
  }, []);
  ...
  return (
    <>
      {startupSplashPhase !== 'hidden' ? (
        <div className={`tilderStartupSplash ${startupSplashPhase === 'exiting' ? 'is-exiting' : 'is-visible'}`}>
          <div className="tilderStartupSplashCard">
            <div className="tilderStartupSplashMark">
            <img className="tilderStartupSplashIcon" src={tilderIcon} alt="Tilder" />
            </div>
            <div className="tilderStartupSplashTitle">Tilder</div>
            <div className="tilderStartupSplashSubtitle">Loading workspace...</div>
            <div className="tilderStartupSplashSpinner" />
            <div className="tilderStartupSplashBar">
              <span className="tilderStartupSplashBarFill" />
            </div>
          </div>
        </div>
      ) : null}
      <ConfirmDialog
        ...
        onConfirm={(remember) => resolveConfirmation(true, remember)}
      />
      <PromptDialog
      ...
      <CommandPalette
        ...
        onRunCommand={(command) => {
          ...
          if (command.type === 'file') {
            handleOpenSearchResult({ path: command.path, name: command.label });
            return;
          }
          ...
          const preferredEditor = getPreferredEditor();
          if (preferredEditor && preferredEditor.getAction(command.id)) {
            preferredEditor.getAction(command.id).run();
            return;
          }
          // Extension command
          if (window.__tilder_extensions?.executeCommand) {
            window.__tilder_extensions.executeCommand(command.id).catch(err => {
              console.warn('Extension command execution failed:', err);
            });
          }
        }}
      />
      <ThemePicker
        ...
        activeTheme={settings?.theme?.active || 'tilder-formal'}
        onSelectTheme={(themeId) => {
          updateSetting('theme.active', themeId);
        }}
      />
      ...
      {pendingTrustFolder && (
        <WorkspaceTrustModal
          folderName={pendingTrustFolder.name}
          onTrust={() => {
            const trustedWorkspaces = JSON.parse(localStorage.getItem('trustedWorkspaces') || '{}');
            trustedWorkspaces[pendingTrustFolder.path] = true;
            localStorage.setItem('trustedWorkspaces', JSON.stringify(trustedWorkspaces));
            workspace.isTrusted = true;
            setPendingTrustFolder(null);
          }}
          onDeny={() => {
            workspace.isTrusted = false;
            setPendingTrustFolder(null);
      ...
      <Modal isOpen={modalOpen} closeModal={closeCurrentModal} title={modalType}>
        ...
        />
        <SystemMonitor modalType={modalType} />
      </Modal>
      <div id="mainProductivityArea" className={`${zenMode ? 'zen-mode' : ''} ${(centeredLayout || zenMode) ? 'centered-layout' : ''}`}>
        {showMenuBar && !zenMode ? (
        <MenuBar
          ...
          triggerNewWindow={handleNewWindow}
          triggerNewTab={handleCreateUntitledFile}
          ...
          openRecentFile={handleOpenRecentFile}
          openRecentWorkspace={handleOpenRecentWorkspace}
          triggerOpenFolder={handleOpenFolder}
          addFolderToWorkspace={handleAddFolderToWorkspace}
          ...
          checkForUpdates={() => handleCheckForUpdates(true)}
          ...
          openProblems={() => openBottomPanel('problems')}
          openOutput={() => openBottomPanel('output')}
          openDebugConsole={() => openBottomPanel('debugConsole')}
          ...
          toggleStatusBar={toggleStatusBarVisibility}
          toggleMenuBar={toggleMenuBarVisibility}
          toggleCenteredLayout={toggleCenteredLayout}
          toggleZenMode={toggleZenMode}
          toggleBreadcrumbs={() => commandHandlers['breadcrumbs.toggle']()}
          toggleMinimap={() => commandHandlers['editor.action.toggleMinimap']()}
          toggleWordWrap={() => commandHandlers['editor.action.toggleWordWrap']()}
          reopenClosedEditor={handleReopenClosedEditor}
          zenModeEnabled={zenMode}
          centeredLayoutEnabled={centeredLayout || zenMode}
          statusBarVisible={showStatusBar && !zenMode}
          menuBarVisible={showMenuBar && !zenMode}
          sidebarVisible={!zenMode && !!activePanel}
          panelVisible={!zenMode && terminalOpen}
          breadcrumbsVisible={breadcrumbsEnabled}
          minimapVisible={settings.minimap?.enabled !== false}
          wordWrapEnabled={settings.wordWrap !== 'off'}
        ...
        <div className="mainsect">
          <div className="codewrpr">
            <div className={`maincodearea ${terminalOpen ? 'terminal-open' : ''}`} ref={mainCodeAreaRef}>
              {workspace.isTrusted === false && (
                <div className="restricted-mode-banner" style={{ background: 'rgba(229, 41, 32, 0.12)', borderBottom: '1px solid rgba(229, 41, 32, 0.3)', padding: '6px 12px', color: '#ffb3b0', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                  <i className="fa-solid fa-shield-halved" style={{ color: '#ff827d' }}></i>
                  <span><strong>Restricted Mode</strong> is intended for safe code browsing. Trust this workspace to enable all features.</span>
                  <button style={{ marginLeft: 'auto', background: 'rgba(229, 41, 32, 0.2)', border: '1px solid rgba(229, 41, 32, 0.4)', padding: '4px 10px', borderRadius: '6px', color: 'white', cursor: 'pointer', fontSize: '0.8rem' }} onClick={() => setPendingTrustFolder({ name: workspace.rootName || 'Workspace', path: workspace.rootPath })}>Manage</button>
                </div>
              )}
              {!splitEditorOpen ? (
                <>
              ...
              <div className={`editor-preview-row ${livePreviewOpen && showLivePreviewAction ? 'preview-open' : ''}`}>
                <div className={`editor-preview-main ${splitEditorOpen ? 'split-open' : ''}`}>
                  ...
                  >
                    ...
                    >
                      {debugSession && (debugSession.status === 'running' || debugSession.status === 'paused') && (
                        <div className="editor-debug-floating-toolbar">
                          <div className="debug-toolbar-drag-handle">
                          ...
                          <button
                            title={debugSession.status === 'paused' ? 'Continue (F5)' : 'Pause (F6)'}
                            onClick={() => debugSession.status === 'paused' ? debugApi.resume() : debugApi.pause()}
                            className="debug-toolbar-btn continue-pause-btn"
                          >
                            {debugSession.status === 'paused' ? (
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" style={{color: '#388e3c'}}>
                                <path d="M4.5 3a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .8.4l7-4.5a.5.5 0 0 0 0-.8l-7-4.5A.5.5 0 0 0 4.5 3z" />
                              </svg>
                            ) : (
                              <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" style={{color: '#0288d1'}}>
                          ...
                          <button
                            ...
                            className="debug-toolbar-btn step-over-btn"
                            disabled={debugSession.status !== 'paused'}
                          >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                              <path d="M11 3.5a.5.5 0 0 1 .5.5v1.5a2.5 2.5 0 0 1-2.5 2.5H4.207l1.354 1.354a.5.5 0 0 1-.708.708l-2.2-2.2a.5.5 0 0 1 0-.708l2.2-2.2a.5.5 0 0 1 .708.708L4.207 7H9a1.5 1.5 0 0 0 1.5-1.5V4a.5.5 0 0 1 .5-.5z"/>
                            </svg>
                          </button>
                          <button
                            title="Step Into (F11)"
                            onClick={() => debugApi.stepInto()}
                            className="debug-toolbar-btn step-into-btn"
                            disabled={debugSession.status !== 'paused'}
                          >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                              <path d="M8 1a.5.5 0 0 1 .5.5v8.793l2.146-2.147a.5.5 0 0 1 .708.708l-3 3a.5.5 0 0 1-.708 0l-3-3a.5.5 0 1 1 .708-.708L7.5 10.293V1.5A.5.5 0 0 1 8 1zm-6 13.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"/>
                            </svg>
                          </button>
                          <button
                            title="Step Out (Shift+F11)"
                            onClick={() => debugApi.stepOut()}
                            className="debug-toolbar-btn step-out-btn"
                            disabled={debugSession.status !== 'paused'}
                          >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                              <path d="M8 15a.5.5 0 0 1-.5-.5V5.707L5.354 7.854a.5.5 0 1 1-.708-.708l3-3a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1-.708.708L8.5 5.707V14.5a.5.5 0 0 1-.5.5zm-6-13.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"/>
                            </svg>
                          </button>
                          <button
                            title="Restart"
                            ...
                            className="debug-toolbar-btn restart-btn"
                          >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" style={{color: '#4caf50'}}>
                              <path d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2v1z" />
                              <path d="M8 4.5a.5.5 0 0 1-.5-.5V1a.5.5 0 0 1 1 0v3a.5.5 0 0 1-.5.5z" />
                              <path d="M6 3.5a.5.5 0 0 1-.5-.5V2a.5.5 0 0 1 1 0v1a.5.5 0 0 1-.5.5z" />
                            </svg>
                          </button>
                          <button
                            title="Stop (Shift+F5)"
                            ...
                            className="debug-toolbar-btn stop-btn"
                          >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" style={{color: '#f44336'}}>
                      ...
                      {splitEditorOpen && primaryPaneEditorTab && breadcrumbsEnabled ? (
                        <BreadcrumbsBar
  ...
  <MonacoEditor
                            key={`${activeTab.id}:${activeTab.language}:${activeTab.name}`}
                            tab={activeTab}
                            ...
                            onEscapeCurrentMode={escapeCurrentMode}
                            onGoToLine={handleGoToLine}
                            onGoToDefinition={handleGoToDefinition}
                            onGoToReferences={handleGoToReferences}
                          onPeekDefinition={handlePeekDefinition}
                          onPeekReferences={handlePeekReferences}
                          onRenameSymbol={handleRenameSymbol}
                          intelliSense={getEditorIntelliSense(activeTab)}
                          lspBridge={getEditorLspBridge(activeTab)}
                          settings={settings}
  ...
  <MonacoEditor
                          ...
                          onPeekReferences={handlePeekReferences}
                          onRenameSymbol={handleRenameSymbol}
                          intelliSense={getEditorIntelliSense(primaryPaneEditorTab)}
                          lspBridge={getEditorLspBridge(primaryPaneEditorTab)}
                          settings={settings}
                      ...
                      ? splitPaneGroups.map((group, paneIndex) => {
                          ...
                          return (
                            <Fragment key={`split-pane-${group.id}-${paneTabId || 'empty'}`}>
                              ...
                              />
                              {paneTab ? (
                                  isOfficeFile(paneTab.name) ? (
  <OfficeEditor tab={paneTab} onChange={(value) => { workspace.updateTabContent(paneTab.id, value ?? ''); refresh(); queueSave(paneTab.id); }} />
) : (
  <MonacoEditor
                                    key={`secondary-${paneIndex}-${paneTab.id}:${paneTab.language}:${paneTab.name}`}
                                    tab={paneTab}
                                    onChange={(value) => {
              ...
              <Terminal
                  isOpen={terminalOpen}
                  height={terminalMaximized ? 460 : terminalHeight}
                  isMaximized={terminalMaximized}
                activeView={bottomPanelView}
                onChangeView={setBottomPanelView}
                ...
                debugConsoleEntries={debugConsoleEntries}
                onEvaluateDebug={handleEvaluateDebug}
                debugSession={debugSession}
                onClearDebugConsole={() => setDebugConsoleEntries([])}
                settings={settings}
              />
            </div>
          </div>
          ...
          <div className="SideBarmainwrper" style={sideBarWrapperStyle} ref={sideBarWrapperRef}>
            ...
            <GitHub
              ...
              authSession={authSession}
              openAccount={openAccount}
              handleCloneGitHubRepo={handleCloneGitHubRepo}
              workspace={workspace}
              pushNotification={pushNotification}
              onRemoveWatch={handleRemoveWatch}
              settings={settings}
              callStack={activeDebugStack}
              variables={activeDebugVariables}
              consoleEntries={debugConsoleEntries}
              onEvaluate={handleEvaluateDebug}
            />
            <Debug
              ...
              onSetBreakpointCondition={handleSetBreakpointCondition}
              onAddWatch={handleAddWatch}
              onRemoveWatch={handleRemoveWatch}
              settings={settings}
            />
            <TestRunner
              ariaExpandedisplaytestrunner={panelDisplay('testrunner')}
              pushNotification={pushNotification}
            />
            <Search
              ...
              onSearchReplaceComplete={handleSearchReplaceComplete}
              onGoToLine={handleGoToLine}
            ...
            <FilePioneer
              ...
              closeTab={handleCloseTab}
              forceCloseTab={forceCloseTabSilently}
              refresh={refresh}
              activeTabId={workspace.activeTabId}
              triggerOpenFolder={handleOpenFolder}
              createFolderRequestNonce={createFolderRequestNonce}
              createFileRequestNonce={createFileRequestNonce}
              renameRequestNonce={renameRequestNonce}
              confirmDelete={handleDeleteRequest}
              pushNotification={pushNotification}
              revealActiveFile={handleRevealActiveFile}
              copyPath={() => handleCopyExplorerPath()}
              copyRelativePath={() => handleCopyExplorerPath({ relative: true })}
            ...
            <Backpack
              ...
              snippets={backpackSnippets}
              onChange={handleBackpackSnippetsChange}
            />
            ...
            <SideBar
              ...
              showTests={builtInFeatures.runDebug}
              ...
              toggleAriaExpandedtestrunner={() => toggleSidebarPanel('testrunner')}
              ...
              toggleAriaExpandedbackpack={() => toggleSidebarPanel('backpack')}
            />
          </div>
          ) : null}
        </div>
          ...
          >
            <div className="go-popup" onMouseDown={(event) => event.stopPropagation()}>
              ...
              <div className="go-popup-actions">
                ...
                <button
                  type="button"
                  className={`go-popup-chip ${goOverlay.mode === 'definition' ? 'active' : ''}`}
                ...
                <button
                  type="button"
                  className={`go-popup-chip ${goOverlay.mode === 'reference' ? 'active' : ''}`}
                  onClick={handlePeekReferences}
                >
                  References
                </button>
              </div>
              <div className="go-popup-body">
                  ...
                  {goOverlay.mode === 'line'
                    ...
                    : goOverlay.mode === 'file'
                      ...
                      : goOverlay.mode === 'workspace-symbol'
                        ...
                        : goOverlay.mode === 'definition'
                          ? 'Peek definition results for the current symbol. Press Enter to jump to the first one.'
                          : goOverlay.mode === 'reference'
                            ? 'Peek references for the current symbol. Press Enter to jump to the first one.'
                ...
                <div className="go-popup-input-row">
                  <input
                    ...
                    onChange={(event) => {
                      if (goOverlay.mode === 'definition' || goOverlay.mode === 'reference') {
                        return;
                      }
                      setGoOverlay((current) => ({ ...current, value: event.target.value }));
                    }}
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') {
                        event.preventDefault();
                    ...
                    readOnly={goOverlay.mode === 'definition' || goOverlay.mode === 'reference'}
                    placeholder={
                      goOverlay.mode === 'line'
                        ? '12 or 12:4'
                        : goOverlay.mode === 'file'
                    ...
                    <div className="go-popup-hint">
                      {goOverlay.loading
                        ...
                        : goOverlay.error
                          ? goOverlay.error
                          : goOverlay.mode === 'definition'
                            ? `${goOverlay.results.length} definition match${goOverlay.results.length === 1 ? '' : 'es'}`
                            : goOverlay.mode === 'reference'
                              ? `${goOverlay.results.length} reference match${goOverlay.results.length === 1 ? '' : 'es'}`
                    ...
                    <div className="go-popup-results">
                      {goOverlay.results.length ? (
                        ...
                        <div className="go-popup-empty">
                          {goOverlay.mode === 'definition' || goOverlay.mode === 'reference'
                            ? 'No navigation results yet for the current symbol.'
                            : goOverlay.value.trim()
        ...
        <StatusBar
          ...
          currentLanguageCapability={
            activeTab?.language ? editorCapabilities?.languages?.[activeTab.language] || null : null
          }
          runtimeMode={editorCapabilities?.runtimeMode || 'unknown'}
          rootName={workspace.rootName || workspace.getRootNode()?.name || ''}
          ...
          settings={settings}
          detectedRuntimes={detectedRuntimesState}
          customRuntimes={customRuntimes}
          onSelectCompiler={(lang, runtime) => {
            if (runtime === null) {
              setCustomRuntimes((prev) => { const next = { ...prev }; delete next[lang]; return next; });
            } else {
              setCustomRuntimes((prev) => ({ ...prev, [lang]: runtime }));
            }
          }}
        />
        ) : null}
