...
// ─── Pulse Lens: external browser log receiver & Live Server ───────────────
app.options('/api/pulse-lens/log', (req, res) => {
...
app.post('/api/pulse-lens/log', (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  const { msg, path: filePath, line, isError, method } = req.body || {};
  io.emit('pulse-lens:log', { msg, path: filePath, line, isError: Boolean(isError), method: method || 'log' });
  res.json({ ok: true });
});

// Serve the Pulse Lens client script for external browsers
app.get('/pulse-lens.js', (req, res) => {
  ...
  // Live Reload logic
  var sse = new EventSource('http://localhost:${serverPort}/live/watch?file=' + encodeURIComponent(_file));
...
app.get('/live/watch', (req, res) => {
  ...
  let watcher = null;
  try {
    watcher = fsSync.watch(targetPath, (eventType) => {
      if (eventType === 'change') {
        res.write('data: reload\n\n');
      }
    });
  } catch (err) {
    // If file doesn't exist yet or can't be watched, just ignore gracefully
  }
...
app.get('/live', async (req, res) => {
  ...
  try {
    ...
    if (ext === '.html' || ext === '.htm') {
      // Pulse Lens-style Source Code Instrumentation
