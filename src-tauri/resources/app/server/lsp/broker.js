export function createLspBroker({
  ...
  function startSessionProcess(session) {
    ...
    if (!session.process) {
      emitStatus(session, {
        status: 'error',
        languageId: session.languageId,
        message: 'Failed to start language server process.',
      });
      sessions.delete(session.key);
      return;
    }
    ...
    session.process.stdout.on('data', (chunk) => {
      session.stdoutBuffer = parseMessages(
        ...
        (message) => {
          const requestId = Object.prototype.hasOwnProperty.call(message || {}, 'id') ? String(message.id) : '';
          const ownerSocketId = requestId ? session.requestOwners.get(requestId) : '';

          if (requestId) {
            console.log(`[LSP Broker] Sending response for request ID ${requestId} to client ${ownerSocketId}:`, JSON.stringify(message).slice(0, 1000));
            session.requestOwners.delete(requestId);
          }

          if (ownerSocketId) {
            namespace.to(ownerSocketId).emit('lsp:message', message);
            return;
          }

          namespace.to(session.room).emit('lsp:message', message);
        }
      );
    });
  ...
  async function ensureSession(languageId, workspaceRoot, adapter) {
    ...
    let command = await resolveInstalledCommand(adapter.commands || []);
    
    if (!command && typeof getDownloadedLspPath === 'function') {
      command = getDownloadedLspPath(languageId);
    }
    
    if (!command && typeof downloadLsp === 'function') {
      emitStatus({ key, languageId, workspaceRoot: normalizedWorkspaceRoot }, {
        status: 'downloading',
        languageId,
        message: `Auto-installing Language Server for ${languageId}... This may take a minute.`
      });
      try {
        command = await downloadLsp(languageId);
      } catch (err) {
        throw new Error(`Failed to auto-install language server: ${err.message}`);
      }
    }
  ...
  function attachSocket(socket, session) {
    ...
    socket.on('lsp:message', (message) => {
      ...
      if (message && Object.prototype.hasOwnProperty.call(message, 'id')) {
        console.log(`[LSP Broker] Received message with ID ${message.id} from socket ${socket.id}:`, JSON.stringify(message));
        session.requestOwners.set(String(message.id), socket.id);
      }
    ...
    socket.on('disconnect', () => {
      ...
      for (const [requestId, ownerSocketId] of session.requestOwners.entries()) {
